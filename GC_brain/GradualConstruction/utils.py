...
def load_data(data_path,TEXT=None):
    ...
    if 'brain' in data_path:
        org_data = cv2.imread(data_path, 0)

        org_data = org_data.reshape(1, 96, 96)
        org_data = image_normalization(org_data)
        org_data_tensor = torch.tensor(org_data).float().to(device)
        org_data_tensor = torch.reshape(org_data_tensor, (1, 1, 96, 96))
