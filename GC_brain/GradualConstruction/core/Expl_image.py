...
class Expl_image(Expl_base):
    ...
    def build(self):
        ...
        sample_per_file = "{}.jpeg".format(str(torch_print(pred_label)))

        org_data_for_save = cv2.normalize(np.array(self.org_data_tensor.data.cpu()), None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        org_data_for_save = org_data_for_save.reshape(96,96,1)
        composite_for_save = cv2.normalize(np.array(composite_tensor.data.cpu()), None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        composite_for_save = composite_for_save.reshape(96,96,1)
        # vutils.save_image(org_data_for_save, self.saved_path+org_save_file)
        # vutils.save_image(composite_tensor.data.clone(),'./result/MNIST_cf/'+per_save_file)
        # vutils.save_image(composite_tensor.data.clone(),self.saved_path+per_save_file)
        cv2.imwrite(self.saved_path+org_save_file, org_data_for_save)
        cv2.imwrite(self.saved_path+per_save_file, composite_for_save)

        print('image saved', self.saved_path+org_save_file)
        print('image saved', self.saved_path+per_save_file)
        print('\n'+'-' * 40)
