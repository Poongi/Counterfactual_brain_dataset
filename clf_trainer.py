...
## black image erase code
# black_image = 0
# for i in range(trn_nc[0].shape[1]):
#     if np.all(trn_nc[0][:, i, :] == -1) == True:
#         black_image += 1
...
for epoch in range(num_epochs) : 
    ...
    if max < acc :
        ...
        torch.save(model.module.state_dict(), './models/resnet_18_clf.pt')
    ...
    with torch.no_grad():
        ...
        last_model.load_state_dict(torch.load('./models/resnet_18_clf.pt'))
